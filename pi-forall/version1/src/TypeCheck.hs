...
tcTerm t@(If t1 t2 t3) mty = do
  ...
  ty2 <- tcTerm t2 mty
  ty3 <- tcTerm t3 mty
  unless (Unbound.aeq ty2 ty3) $ Env.err [DS "Types don't match", DD ty2, DS "and", DD ty3]
  return ty3 

tcTerm (Let rhs bnd) mty = do
  tyRhs <- inferType rhs
  (x , tm) <- Unbound.unbind bnd
  Env.extendCtx (mkSig x tyRhs) (tcTerm tm mty)
...
tcTerm t@(Prod a b) (Just ty@(Sigma tyA bnd)) = do
  tcType ty
  checkType a tyA
  checkType b (Unbound.instantiate bnd [a])
  return ty

tcTerm t@(LetPair p bnd) (Just ty) = do
  tyP <- inferType p
  let ensureSigma :: Type -> TcMonad (Type, Unbound.Bind TName Type)
      ensureSigma (Ann a _) = ensureSigma a
      ensureSigma (Pos _ a) = ensureSigma a
      ensureSigma (Sigma tySigA tySigBnd) = return (tySigA, tySigBnd)
      ensureSigma tyErr = Env.err [DS "Expected a pi type but found ", DD tyErr]
  (tyA, tyBnd) <- ensureSigma tyP
  ((x, y), tmBody) <- Unbound.unbind bnd
  Env.extendCtxs [mkSig x tyA, mkSig y (Unbound.instantiate bnd [Var x])] (checkType tmBody ty)
  return ty
